...
In figura~\ref{fig:architettura} è riportata una vista architetturale ad alto livello del sistema. La figura non riporta le interfacce di \textit{callback} per non renderne difficile la fruizione. La parte riguardate l'invio di messaggi dalla componente \evdisp{} verso il resto del sistema viene infatt trattata in dettaglio nella sezione~\ref{sec:dispatcherImpl}, mentre l'uso delle interfacce di \textit{callback} da parte di \sched{} viene discusso in~\ref{sec:scheduler}.
