...
Per effettuare la configurazione del prototipo in base al proprio sistema è necessario eseguire il comando \texttt{configure} ed eventualmente impostare i seguenti parametri utili qualora la procedura automatica non riesca ad individuare le componenti necessarie:
\begin{itemize}
...
\end{itemize}
La compilazione può dunque essere eseguita tramite il comando \texttt{make}.

\subsection*{Passo 1 - \texttt{.hosts.erlang}}
Per prima cosa bisogna inserire i nomi degli \textsl{hosts} che faranno parte del sistema nel file \texttt{.hosts.erlang} posto nella medesima \textsl{directory} degli script di avvio del prototipo.
...
Per avviare le componenti grafiche del prototipo è necessario inizializzare la variabile d'ambiente \texttt{FORSE\_NS} con l'\textsl{hostname} dell'elaboratore in cui è stato avviata la componente \texttt{control\_panel} qualora non coincida con l'\textsl{host} locale.
...
L'avvio \texttt{team\_monitor} è opzionale e può essere effettuato tramite lo \textit{script} \texttt{start\_team\_monitor}. Ogni istanza di tale componente viene associata ad una sola scuderia scelta dall'utente in fase di avvio.
